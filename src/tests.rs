...
async fn test_runtime_upgrade() -> Result<(), Error> {
    ...
    // leaving the line below for reference but disabled because it is frustrating to keep it up to date.
    ...
    // assert_eq!(honest.reports("substrate_build_info{name=\"honest-0\",version=\"1.6.0-5c3e98e9d3e\",chain=\"westend_local_testnet\"}").await?, 1_f64);
...
// -p test-parachain-adder-collator -p test-parachain-undying-collator -p polkadot-test-malus -p polkadot -p polkadot-parachain-bin`.
// List of the required binaries:
